...
### Switching between PMTiles and Martin
  ...
  SEO and indexing:
  - `npm run build` generates route-specific HTML for `/`, `/marchabilite`, and `/cyclabilite`
  - `robots.txt` and `sitemap.xml` are generated in `build/` from `VITE_SITE_URL`
  - Submit `${VITE_SITE_URL}/sitemap.xml` in Google Search Console after deployment
  - Keep clean URLs (`/marchabilite`, `/cyclabilite`) as the public links; legacy hash URLs redirect client-side
  ...
  - If the app shows mismatched colors or attributes after data changes:
    ...
    - Ensure `public/data/attribute_thresholds.json` was regenerated by `npm run tile` or `npm run tile:thresholds`
    - Confirm `.env.local` points to the intended source (PMTiles vs Martin)
    - If a PMTiles file was replaced at the same URL, bump `VITE_TILES_VERSION` and redeploy so the app requests a new cache key
